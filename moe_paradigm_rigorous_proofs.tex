...
\textbf{算法伪代码}：
\begin{verbatim}
算法1：Lipschitz常数 $L_g$ 的标定
输入：验证集 $\mathcal{D} = \{x_i\}_{i=1}^N$，扰动参数 $\varepsilon$
输出：$L_g^{\max}$, $L_g^{0.95}$, $L_g^{\text{mean}}$

1. ratios = []
2. for each $x_i \in \mathcal{D}$:
3.     $\mathbf{e}_i$ = embed($x_i$)  // 获取embedding
4.     $\mathbf{e}_i'$ = $\mathbf{e}_i$ + $\varepsilon \cdot \mathcal{N}(0, I)$  // 添加噪声
5.     $x_i'$ = decode($\mathbf{e}_i'$)  // 重新编码
6.     $\ell_i$ = gating_network($x_i$)  // 计算logits
7.     $\ell_i'$ = gating_network($x_i'$)
8.     $\Delta \ell_i$ = ||$\ell_i - \ell_i'$||$_2$
9.     $\Delta x_i$ = ||$\mathbf{e}_i - \mathbf{e}_i'$||$_2$
10.    if $\Delta x_i > 0$:
11.        ratios.append($\Delta \ell_i / \Delta x_i$)
12. $L_g^{\max}$ = max(ratios)
13. $L_g^{0.95}$ = percentile(ratios, 95)
14. $L_g^{\text{mean}}$ = mean(ratios)
15. return $L_g^{\max}$, $L_g^{0.95}$, $L_g^{\text{mean}}$
\end{verbatim}
...
\begin{verbatim}
算法2：综合常数 $C$ 的标定
输入：验证集 $\mathcal{D} = \{x_i\}_{i=1}^N$，paraphrase函数 $P$
输出：$C_{\text{prop}}$, $C_{\text{stability}}$, $C$

1. gammas = [], deltas = []
2. for each $x_i \in \mathcal{D}$:
3.     $x_i'$ = $P(x_i)$  // 生成paraphrase
4.     $\gamma_i$ = compute_kl_divergence($x_i$, $x_i'$)
5.     $p_i$ = compute_activation_dist($x_i$)  // 方式1
6.     $p_i'$ = compute_activation_dist($x_i'$)
7.     $\delta_i$ = TV_distance($p_i$, $p_i'$)
8.     gammas.append($\gamma_i$)
9.     deltas.append($\delta_i$)
10. 
11. // 健壮回归
12. sqrt_gammas = sqrt(gammas)
13. $C_{\text{prop}}$, $R^2$ = robust_regression(sqrt_gammas, deltas)
14. 
15. // 拟合 $C_{\text{stability}}$
16. chernoff_diffs = []
17. for each $(x_i, x_i')$:
18.     $D*_i$ = compute_chernoff_info($p_i$, $q_i$)
19.     $D*_i'$ = compute_chernoff_info($p_i'$, $q_i'$)
20.     chernoff_diffs.append(|$D*_i' - D*_i$|)
21. $C_{\text{stability}}$ = fit_stability_constant(deltas, chernoff_diffs)
22. 
23. $C$ = $C_{\text{stability}} \cdot C_{\text{prop}}$
24. return $C_{\text{prop}}$, $C_{\text{stability}}$, $C$
\end{verbatim}
...
\begin{verbatim}
算法3：安全系数 $c$ 的最优标定
输入：验证集 $\mathcal{D}$，攻击强度 $\gamma$，权重 $\lambda$，检测精度 $\delta$
输出：最优安全系数 $c^*$

1. // 步骤1：标定性能成本函数
2. $c_{\text{scan}}$ = [C-0.2, C-0.1, ..., 2.5C+0.2]  // 步长0.1
3. performance_costs = []
4. for each $c \in c_{\text{scan}}$:
5.     embed_watermark($c$)  // 嵌入水印
6.     $\Delta A(c)$ = measure_performance_degradation($\mathcal{D}$)
7.     performance_costs.append($\Delta A(c)$)
8. $a, b, p, q$ = fit_polynomial($c_{\text{scan}}$, performance_costs)
9. $\Delta A(c)$ = $a \cdot c^p + b \cdot c^q$
10.
11. // 步骤2：网格搜索
12. $c^*$ = None
13. min_objective = $\infty$
14. 
15. // 第一轮：粗网格
16. $c_{\text{coarse}}$ = [C, C+0.2, ..., 2.5C]  // 步长0.2
17. for each $c \in c_{\text{coarse}}$:
18.     $n^*(c)$ = log(1/$\delta$) / ($\gamma \cdot c \cdot (c-C)$)
19.     objective = $n^*(c)$ + $\lambda \cdot \Delta A(c)$
20.     if objective < min_objective:
21.         min_objective = objective
22.         $c^*$ = $c$
23.
24. // 第二轮：细网格
25. $c_{\text{fine}}$ = [$c^*$-0.4, $c^*$-0.35, ..., $c^*$+0.4]  // 步长0.05
26. for each $c \in c_{\text{fine}}$:
27.     $n^*(c)$ = log(1/$\delta$) / ($\gamma \cdot c \cdot (c-C)$)
28.     objective = $n^*(c)$ + $\lambda \cdot \Delta A(c)$
29.     if objective < min_objective:
30.         min_objective = objective
31.         $c^*$ = $c$
32.
33. // 第三轮：精细搜索（可选）
34. $c_{\text{precise}}$ = [$c^*$-0.1, $c^*$-0.09, ..., $c^*$+0.1]  // 步长0.01
35. for each $c \in c_{\text{precise}}$:
36.     $n^*(c)$ = log(1/$\delta$) / ($\gamma \cdot c \cdot (c-C)$)
37.     objective = $n^*(c)$ + $\lambda \cdot \Delta A(c)$
38.     if objective < min_objective:
39.         min_objective = objective
40.         $c^*$ = $c$
41.
42. return $c^*$, min_objective
\end{verbatim}
