...
\begin{tabular}{lccc}
\toprule
\textbf{攻击方法} & \textbf{平均编辑距离} & $\gamma_{\text{upper}}$ (nats) & $\gamma_{\text{measured}}$ (nats) \\
\midrule
...
\begin{tabular}{lcc}
\toprule
$\gamma$ (nats) & \textbf{范式A (Token-Logit)} & \textbf{范式B (MoE Expert)} \\
 & Z-score / 可检测性 & D* / 可检测性 \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
\textbf{模型} & $L_g^{\max}$ & $L_g^{0.95}$ & $L_g^{\text{mean}}$ & \textbf{理论假设} \\
\midrule
...
\begin{tabular}{lcccc}
\toprule
$c$ & $n*(\gamma,c)$ @ $\gamma=0.03$ & $\Delta A(c)$ (PPL) & \textbf{目标函数值} & \textbf{是否最优} \\
 & (99\% 检测) & & ($\lambda=1$) & \\
\midrule
1.5 & 1250 & 0.8\% & 1251 & — \\
...
{\footnotesize
\begin{verbatim}
Algorithm 1: Calibrate Lipschitz constant L_g
Input: validation set D, perturbation epsilon
Output: L_g_max, L_g_095, L_g_mean

1. ratios = []
2. for each x_i in D:
3.     e_i = embed(x_i)                    // Get embedding
4.     e_i_prime = e_i + epsilon * N(0,I)  // Add noise
5.     x_i_prime = decode(e_i_prime)       // Re-encode
6.     l_i = gating_network(x_i)           // Compute logits
7.     l_i_prime = gating_network(x_i_prime)
8.     delta_l_i = norm(l_i - l_i_prime)
9.     delta_x_i = norm(e_i - e_i_prime)
10.    if delta_x_i > 0:
11.        ratios.append(delta_l_i / delta_x_i)
12. L_g_max = max(ratios)
13. L_g_095 = percentile(ratios, 95)
14. L_g_mean = mean(ratios)
15. return L_g_max, L_g_095, L_g_mean
\end{verbatim}
}
...
{\footnotesize
\begin{verbatim}
Algorithm 2: Calibrate composite constant C
Input: validation set D, paraphrase function P
Output: C_prop, C_stability, C

1. gammas = [], deltas = []
2. for each x_i in D:
3.     x_i_prime = P(x_i)                    // Generate paraphrase
4.     gamma_i = compute_kl_divergence(x_i, x_i_prime)
5.     p_i = compute_activation_dist(x_i)    // Method 1
6.     p_i_prime = compute_activation_dist(x_i_prime)
7.     delta_i = TV_distance(p_i, p_i_prime)
8.     gammas.append(gamma_i)
9.     deltas.append(delta_i)
10. 
11. // Robust regression
12. sqrt_gammas = sqrt(gammas)
13. C_prop, R_squared = robust_regression(sqrt_gammas, deltas)
14. 
15. // Fit C_stability
16. chernoff_diffs = []
17. for each (x_i, x_i_prime):
18.     D_star_i = compute_chernoff_info(p_i, q_i)
19.     D_star_i_prime = compute_chernoff_info(p_i_prime, q_i_prime)
20.     chernoff_diffs.append(abs(D_star_i_prime - D_star_i))
21. C_stability = fit_stability_constant(deltas, chernoff_diffs)
22. 
23. C = C_stability * C_prop
24. return C_prop, C_stability, C
\end{verbatim}
}
...
{\footnotesize
\begin{verbatim}
Algorithm 3: Optimal calibration of safety coefficient c
Input: validation set D, attack strength gamma, weight lambda,
       detection precision delta
Output: optimal safety coefficient c_star

1. // Step 1: Calibrate performance cost function
2. c_scan = [C-0.2, C-0.1, ..., 2.5*C+0.2]  // step size 0.1
3. performance_costs = []
4. for each c in c_scan:
5.     embed_watermark(c)                    // Embed watermark
6.     Delta_A_c = measure_performance_degradation(D)
7.     performance_costs.append(Delta_A_c)
8. a, b, p, q = fit_polynomial(c_scan, performance_costs)
9. Delta_A_c = a * power(c, p) + b * power(c, q)
10.
11. // Step 2: Grid search
12. c_star = None
13. min_objective = infinity
14. 
15. // Round 1: Coarse grid
16. c_coarse = [C, C+0.2, ..., 2.5*C]  // step size 0.2
17. for each c in c_coarse:
18.     n_star_c = log(1/delta) / (gamma * c * (c - C))
19.     objective = n_star_c + lambda * Delta_A_c
20.     if objective < min_objective:
21.         min_objective = objective
22.         c_star = c
23.
24. // Round 2: Fine grid
25. c_fine = [c_star-0.4, c_star-0.35, ..., c_star+0.4]  // step 0.05
26. for each c in c_fine:
27.     n_star_c = log(1/delta) / (gamma * c * (c - C))
28.     objective = n_star_c + lambda * Delta_A_c
29.     if objective < min_objective:
30.         min_objective = objective
31.         c_star = c
32.
33. // Round 3: Precise search (optional)
34. c_precise = [c_star-0.1, c_star-0.09, ..., c_star+0.1]  // step 0.01
35. for each c in c_precise:
36.     n_star_c = log(1/delta) / (gamma * c * (c - C))
37.     objective = n_star_c + lambda * Delta_A_c
38.     if objective < min_objective:
39.         min_objective = objective
40.         c_star = c
41.
42. return c_star, min_objective
\end{verbatim}
}
...
\begin{tabular}{lccc}
\toprule
\textbf{模型} & \textbf{参数量} & $c_{\max}$ & \textbf{推荐 $c*$} \\
\midrule
